...
st.title("Mobile Stroke Unit System")

st.write("Welcome to your interactive stroke consultation app.")
...
selected_name = st.selectbox("Choose a patient:", patient_names)
...
st.write(f"**Chief Complaint:** {selected_patient['Chief complaint']}")
...
if st.button("🧠 Run Consultation 🧠"):
    result = consult_patient(selected_patient)
    st.markdown(result)
...
def patient_alerts(patient):
        ...
        if alert.strip() == f"⚠️ Alerts for {patient['Name']} ⚠️:":
          alert += "- No critical alerts found.\n"

        alert += "-" * 50 + "\n" 
        return alert 

if st.button("⚠️ Show Alerts ⚠️"):
    alert_msg = patient_alerts(selected_patient)
    st.markdown(alert_msg)
...
def follow_up(patient):
        ...
        if "ICU" in treatment:
            summary += "- Admit to the ICU.\n"

        if "therapy" in treatment:
            summary += "- Schedule patient for therapy.\n"

        if "monitoring" in treatment:
            summary += "- Place patient under hospital observation.\n"

        if "surgical" in treatment:
            summary += "- Patient requires surgical evaluation.\n"

        summary += "-"*72 + "\n"
        return summary

if st.button("🏥Show Follow-Up Plan🏥"):
  plan = follow_up(selected_patient)
  st.markdown(plan)
...
if st.button("💾 Export Patient Report Summary 💾"):
  report = export_summary(selected_patient)
  st.success(f"Summary exported to file: `{report}`")
